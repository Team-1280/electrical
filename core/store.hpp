...
concept GenericSerializer = requires {
    typename T::IdType;
 ...
 * resources
 */
template<GenericStoreValue... Values>
        ...
        for(const auto& entry : resource_items) {
            if(entry.is_regular_file() && entry.path() != cachefile_path) {
                try {
                    ...
                    elem->second.name = std::move(Serializer<T>::load_name(entry_json));
...
private:
    ...
    inline void construct_single_type() {
        std::filesystem::path cachefile_path = Serializer<T>::RESOURCE_DIR / "cache.json";
        try {
